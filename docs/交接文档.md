...
- 按需格式（renditions）
  ...
  - 新增“单文件下载清单”接口：`GET /api/assets/:uuid/renditions/files?format=obj` 会按需“物化” OBJ/MTL/贴图到 R2 的 `assets/<user>/<asset>/obj/`，并返回“同源代理下载”清单（不再直接暴露 R2 预签名 URL，详见下文“OBJ 预览 403 与代理修复”）。前端 OBJ 下载改为新窗口打开清单并逐个下载（`components/assets/RenditionsPanel.tsx`）。
  - `RENDITIONS_INSTANT_READY_FOR_ORIGINAL=1`：当请求格式与原始一致（例如原始 GLB→生成 GLB）时，直接标记就绪并复用原始文件。

- 存储与 CORS
  - R2 CORS 建议配置（示例）（仅供封面直链或其它前端直连场景使用；OBJ/MTL/贴图预览现已统一走后端代理，不再依赖 CORS 成功与否）：
    [
      {
...
### 六、本次问题与修复说明（2025‑11‑15 夜）——OBJ 预览 403 与 R2 代理

> 现象：OBJ 预览在浏览器控制台报 `TypeError: Failed to fetch`，Network 面板中可以看到指向 `*.r2.cloudflarestorage.com/.../obj/0.obj?...` 的请求返回 403；即使在终端用 `curl` 带浏览器同款头访问该预签名 URL，也得到 403。下载 ZIP 包仍然可用。

- 背景与根因
  - 早期实现中，`GET /api/assets/:uuid/renditions/files?format=obj` 会直接返回 R2 预签名 URL 清单，前端 `ViewerOBJ` 使用 `OBJLoader/TextureLoader` 通过 `fetch` 直连 R2 加载 OBJ/MTL/贴图。
  - 该模式在以下组合下存在隐患：
    - 桶保持“禁止公共访问”（私有）；
    - R2 侧针对 S3 预签名/Referer/Origin 等存在额外限制策略；
    - 或某些签名在实际访问路径上被视为无效；
  - 结果是：预签名 URL 虽然生成成功，但浏览器/`curl` 访问时仍返回 403，导致 OBJ 预览始终失败。
  - CORS 配置在这种场景下不是根因：即便 CORS 放宽为 `*`，403 仍然发生，说明问题在于“预签名直连被拒”，而不是浏览器跨域校验。

- 行为与架构调整
  1. **OBJ/MTL/贴图统一通过后端代理访问**
     - 新增路由：`GET /api/assets/:uuid/renditions/file?name=<文件名>`（同源、带鉴权）。
       - 位置：`app/api/assets/[uuid]/renditions/file/route.ts`。
       - 鉴权：`getUserUuid()` + `findAssetByUuid(uuid)`，仅资产所有者可访问。
       - 参数：
         - `name`：OBJ 目录下的相对文件名，例如 `0.obj`、`0.mtl`、`material_0.png`。
         - 会经过 `sanitizeName()`：清理 `?/#`、前导 `/`、反斜杠与 `../` 等，防止路径穿越。
       - 行为：
         - 构造存储键：`assets/<user_uuid>/<asset_uuid>/obj/<name>`；
         - 通过 `newStorage().getObjectStream({ key })` 从 R2/S3 拉取流；
         - 转换为 WebReadableStream 返回，附带：
           - `Content-Type`（透传 R2 内容类型）；
           - `Content-Length`（如可用）；
           - `Content-Disposition: inline; filename=<name>`；
           - `Cache-Control: private, max-age=300`。
     - 好处：
       - 浏览器只访问自己域名（同源），完全绕过 R2 CORS 与预签名直连策略差异；
       - 桶仍保持私有，仅后端持有 AK/SK 并从内部访问。

  2. **OBJ 文件清单改为返回“同源代理 URL”**
     - 路由：`GET /api/assets/:uuid/renditions/files?format=obj`。
       - 位置：`app/api/assets/[uuid]/renditions/files/route.ts`。
     - 原逻辑：
       - 对 `assets/<user>/<asset>/obj/` 目录下的 OBJ/MTL/贴图逐个调用 `storage.getSignedUrl()`，返回 R2 预签名 URL 清单；
       - 前端直接 `fetch` 这些 URL。
     - 新逻辑：
       - 保持“自动 materialize / ZIP 解包 / cover_key 回填”等行为不变；
       - 但在最终 `files` 构造时：
         - 不再调用 `getSignedUrl()`；
         - 统一返回同源代理地址：
           - `files.push({ name: cleanName, url: \`/api/assets/${asset.uuid}/renditions/file?name=${encodeURIComponent(cleanName)}\` })`
       - 即：`files[i].url` 永远是站点自己的 API 路径，不直接暴露 R2 域名。
     - 影响：
       - `ViewerOBJ` 与 `RenditionsPanel` 均自动受益，不需要前端改动接口约定。

  3. **前端 OBJ 清单缓存版本升级**
     - 组件：`components/assets/AssetAutoPreviewOBJ.tsx`。
     - 为了避免老版本浏览器 sessionStorage 中残留的“R2 预签名 URL 清单”继续被复用，本次将缓存 key 从：
       - 原：`asset-obj-files:<assetUuid>`；
       - 改为：`asset-obj-files:v2:<assetUuid>`。
     - 效果：部署新版本后，浏览器会自动忽略旧缓存，重新请求 `/api/assets/:uuid/renditions/files?format=obj` 获取带同源代理 URL 的新清单。

- 对运维与后续开发的注意事项
  - OBJ/MTL/贴图的在线预览已经不再依赖 R2 CORS 或“浏览器直连 R2 的预签名 URL”，后续即便调整 R2 的公共访问策略，只要后端 AK/SK 有权限，预览仍然可用。
  - 封面图与下载场景依旧可以保留两种模式：
    - 通过 `/api/assets/:uuid/download` 走服务端代理（`STORAGE_DOWNLOAD_MODE=proxy`）；
    - 或通过预签名 URL + `window.location.href` 发起下载（`STORAGE_DOWNLOAD_MODE=presigned`），这不经过 `fetch`，一般不会受到 CORS 影响。
  - 如需在未来再次让浏览器“直连 R2 做流式加载”（例如前端自己做分块下载等），请优先评估：
    - 当前 R2 Bucket Policy 是否允许该模式；
    - 是否真的需要暴露 R2 域名给前端；大多数 3D 预览场景推荐沿用当前“后端代理”方案。
...
## 本次对话增量（2025‑11‑15）—— 品牌统一与用户中心调整

本次迭代聚焦“AI3DMARK 品牌统一、用户中心信息架构调整与法务文案落地”，主要影响站点头部/脚部、用户中心入口、隐私与条款页面，以及 Logo / favicon 素材。

### 一、用户中心导航与入口（Console 区域）

- 用户中心入口指向调整  
  - 位置：`components/sign/user.tsx`。  
  - 变更：头像下拉菜单中的“用户中心”从 `/my-orders` 改为 `/my-assets`，与产品信息架构对齐（默认先看资产而不是订单列表）。  

- Console 侧边栏结构调整  
  - 位置：`app/[locale]/(default)/(console)/layout.tsx`。  
  - 新结构：  
    - 资产管理：`/my-assets`（icon：`RiGalleryLine`）；  
    - 订阅管理：`/subscriptions`（icon：`RiVipCrown2Line`）；  
    - 我的积分：`/my-credits`；  
    - 我的邀请：`/my-invites`。  
  - 隐藏菜单：  
    - “我的订单”（`/my-orders`）和“API 密钥”（`/api-keys`）从侧边导航中移除，但保留路由本身，后续如需要可再暴露。  

- 订阅管理页面占位  
  - 位置：`app/[locale]/(default)/(console)/subscriptions/page.tsx`。  
  - 行为：展示标题“订阅管理”与文案“本站还处于内测阶段，暂不开放充值，可免费使用。”，作为暂未开放充值功能时的统一说明。  

### 二、品牌与文案统一（AI3DMARK / 邀请积分）

- 站点基础元信息  
  - 位置：`i18n/messages/zh.json`、`i18n/messages/en.json` 中的 `metadata`。  
  - 变更：  
    - `metadata.title` 简化为纯产品名 `AI3DMARK`，避免浏览器标签过长；  
    - `metadata.description` 更新为“AI 3D 生成与资产管理平台”的描述（中英文各一版），不再描述为通用 ShipAny 模板。  

- 邀请系统文案改为“积分奖励”  
  - 位置：`i18n/messages/zh.json#my_invites`、`i18n/messages/en.json#my_invites`。  
  - 主要调整：  
    - 中文：  
      - `invite_tip` → “每邀请 1 位朋友成功注册并使用，可获得 50 积分。”；  
      - `invite_balance` → “邀请奖励积分”；  
      - `no_orders` → “你需要先在本站完成一笔付费订单后才能邀请朋友”（去掉 ShipAny 相关表述）。  
    - 英文：  
      - `invite_tip` → “Invite 1 friend to sign up and use the service, earn 50 credits.”；  
      - `invite_balance` → “Invite Reward Credits”；  
      - `no_orders` → “You need to complete at least one paid order before inviting others”。  

- 订单列表描述中性化  
  - 位置：`i18n/messages/zh.json#my_orders`、`i18n/messages/en.json#my_orders`。  
  - 变更：  
    - 中文 `description` 从 “在 ShipAny 上购买的订单。” 改为 “在本站完成的付费订单。”；  
    - 英文 `description` 改为 “Orders you have paid on this site.”。  

### 三、页脚与公司主体信息

- Footer 组件默认版权文案  
  - 位置：`components/blocks/footer/index.tsx`。  
  - 变更：  
    - 默认版权从 `© 2025 • ShipAny 保留所有权利。` 改为 `© 2025 • xyris labs limited 保留所有权利。`；  
    - “build with ShipAny” 替换为 “build with AI3DMARK”，并将链接指回站内 `/`。  

- Landing 配置中的品牌信息  
  - 位置：`i18n/pages/landing/zh.json#footer`、`i18n/pages/landing/en.json#footer`。  
  - 变更：  
    - `brand.title` 统一为 `AI3DMARK`，`brand.description` 改为“AI 3D 生成与资产管理平台”描述；  
    - 版权字段同步替换为 `© 2025 • xyris labs limited 保留所有权利。` / `All rights reserved.`。  

### 四、Landing / 导航行为调整

- 默认首页改为“创作页”  
  - 位置：`app/[locale]/(default)/page.tsx`。  
  - 变更：移除原落地页拼装逻辑，改为服务端直接 `redirect("/generate")`。  
  - 结果：访问根路径（如 `/zh`）会直接进入“创作 3D 模型”页面，原 ShipAny 营销落地页隐藏，仅保留配置文件供后续参考。  

- 去除“获取 ShipAny” 顶部按钮  
  - 位置：`i18n/pages/landing/zh.json#header.buttons`、`i18n/pages/landing/en.json#header.buttons`。  
  - 变更：将 header.buttons 数组置空，不再渲染 “获取 ShipAny / Get ShipAny” 顶部 CTA 按钮；  
  - 定价卡片与 CTA 中残余的 ShipAny 营销文案仍保留用于未来可能的模版复用，但当前由于首页 redirect，不会在正常路径中暴露给用户。  

### 五、法务页面重写（隐私政策 / 服务条款）

- 隐私政策  
  - 位置：`app/(legal)/privacy-policy/page.mdx`。  
  - 变更：  
    - 全文改写为面向 AI3DMARK 的隐私政策，主体为 **xyris labs limited**；  
    - 明确说明收集的信息类别：账户与身份信息、使用与日志信息、设备与网络信息、Cookie、本地存储以及积分/支付相关记录；  
    - 说明使用目的：提供服务、安全与风控、产品改进、履行法律义务；  
    - 描述存储位置（云服务、数据库与对象存储）、安全措施与保留期限；  
    - 描述信息共享的场景（第三方云服务/支付、法律要求等）与用户权利（访问、更正、删除/注销、撤回同意）；  
    - 增加未成年人条款、第三方链接说明以及政策变更与联系渠道（示例邮箱 `support@xyrislabs.com`，可按实际调整）。  

- 服务条款  
  - 位置：`app/(legal)/terms-of-service/page.mdx`。  
  - 变更：  
    - 将原 ShipAny 模板相关内容替换为 AI3DMARK 的网站服务条款；  
    - 包含：账号注册与使用、服务内容（3D 生成 / 资产管理 / 广场等）、用户行为规范、用户内容与平台内容的知识产权说明；  
    - 约定积分与付费服务的基本规则（积分用途、付费与结算、价格变更等），兼容当前“内测阶段暂不开放充值”的状态；  
    - 明确服务中断（维护、故障、不可抗力、上游服务异常）及责任限制条款；  
    - 引用隐私政策：通过 `<Link href=\"/privacy-policy\">隐私政策</Link>` 链接；  
    - 说明适用法律与争议解决方式、条款更新机制以及联系信息（同隐私政策）。  

> 说明：上述法务文本仅为工程侧合理默认版本，不构成法律意见。若后续有正式法务版本，应以法务提供的文本覆盖。

### 六、Logo 与浏览器标签图标（favicon）

- UI 中使用的新 Logo  
  - 位置：`public/logo.svg`。  
  - 设计：  
    - 使用与主题一致的橙色→绿色线性渐变（接近 `--primary` 与绿色配色）；  
    - 图形为圆环外圈 + 中心等距立方体线框，呼应“3D / 立体 / 资产盒子”的概念；  
    - 无大色块背景，更适配当前浅色导航和卡片化 UI。  
  - 替换点：  
    - Landing：`i18n/pages/landing/zh.json` 与 `en.json` 中 brand.logo.src 改为 `/logo.svg`；  
    - 管理后台：`app/[locale]/(admin)/layout.tsx` 中 brand.logo 改为 `/logo.svg`；  
    - 登录/注册页：`app/[locale]/auth/signin/page.tsx` 与 `.../register/page.tsx` 的 logo 图标改为 `/logo.svg`；  
    - UI 测试：`tests/ui/navigation-render.test.tsx` 中 headerConfig.brand.logo.src 改为 `/logo.svg`。  

- 浏览器标签小图标（favicon）  
  - 位置：`app/icon.svg`。  
  - 行为：  
    - 使用与 `logo.svg` 相同的立方体图形，但线条加粗，适配 16x16/32x32 尺寸；  
    - 在 Next App Router 中，`app/icon.svg` 优先作为站点图标，因此浏览器标签会显示新 Logo，而不再是旧的 `favicon.ico` 图标。  

> 维护建议：如后续更换品牌配色或 Logo，可优先替换 `public/logo.svg` 与 `app/icon.svg`，并检查与之耦合的测试与 i18n 配置是否需要同步更新。
